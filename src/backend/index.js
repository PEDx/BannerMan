...
import { getInstanceOrVnodeRect } from './highlighter';
import {
  // stringify,
  classify,
  camelize,
...
export const functionalVnodeMap = new Map();
...
export function scrollIntoView(instance) {
  const rect = getInstanceOrVnodeRect(instance);
  if (rect) {
...
export function bindToConsole(instance) {
  if (!instance) return;
  if (!isBrowser) return;
...
export function setStateValue({ id, path, value, newKey, remove }) {
  const instance = instanceMap.get(id);
  if (instance) {
    try {
      let parsedValue;
